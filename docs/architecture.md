...
| **016** | **`/doctor` reads a public Doctolib endpoint on demand; the request URL is pasted, not discovered** | Logging in and scraping the patient account; re-implementing `agenda_ids` discovery; polling for cancellations; an official API | Narrows, rather than reverses, the x-pulse ruling that "scrapers break and violate ToS". What was rejected there was a *logged-in* scraper standing in for a paid API on every capture. This is one unauthenticated GET of a public availability endpoint per explicitly typed command: no account, no cookies, no credentials, nothing written, nothing booked — so Phase 1's "credentials or payments" and "any action in the world" non-goals stay intact, and "proactive behaviour" rules out the cancellation-watcher this obviously invites. Doctolib publishes no patient API, so the alternative to reading `availabilities.json` is not a supported interface, it is no feature. The brittleness is real and is answered by *not* automating the fragile half: `agenda_ids` appear on no public page, so the user pastes the request their own browser made and VOS overrides only `start_date` and `limit`. A practice that reorganises its calendars is fixed by pasting a new URL, not by shipping code. The endpoint answers `200` with an empty list when those ids are missing, which is indistinguishable from a full calendar — hence a missing-parameter check that raises rather than returns nothing, which is the single most important line in the module |
| **017** | **Reminders make VOS proactive, scoped to one sentence; the scheduler stays inside the daemon** | A third "reminder service" container; a cron job on the host; re-firing until acknowledged; keeping reminder state only in a projection | *(proposed — see `docs/superpowers/specs/2026-08-13-reminders-design.md`)* Phase 1's non-goals rule out "proactive behaviour" wholesale, which was right while nothing had a clock. A reminder needs one. Rather than delete the non-goal, this narrows it to a sentence future features must pass: **the user's own reminder, at the user's own stated time, delivered once.** Anything that speaks first for any other reason is still out. The scheduler is an asyncio ticker in the existing process because the alternative is a second journal writer: firing is something VOS did, but *closing* a reminder is a decision the user authored, which ADR-013 puts in the journal and ADR-008 guarantees has exactly one writer — and ADR-014 already refused to share that append across processes. The `JobQueue` is the seam a separate service would have to reinvent, so the ticker sits in front of it and every guarantee holds; if isolation is ever worth wanting, the cut is at the queue. Telegram is not the obstacle it appears to be — exclusivity is only on `getUpdates`, so a second process could `sendMessage` freely. The journal is the reason, not Telegram. Fire-once rather than nag-until-acknowledged: repeating needs backoff, a cap and quiet hours to avoid a 3am message, and is the behaviour that gets a bot muted. **Refines ADR-013's membership rule.** That rule reads "what VOS's user authored", which leaves a fired reminder homeless: firing is something VOS did, so it could only live in a projection — and `vos reclassify --rebuild` wipes projections, which would re-fire every past reminder. The rule becomes "what VOS's user authored, **and what VOS did that cannot be re-derived**", and `ReminderMark(action="fired")` is the first entry in that second clause. The clause is deliberately narrow: a classification is reconstructed exactly by replay and stays out, while nothing about a reminder's text or due time reveals whether the message actually went out. Sent before recorded, too — the reverse order would let a crash leave a reminder VOS believes it delivered and never did, where this order costs at worst one duplicate |
